...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let rolling_windows = Arc::new(parking_lot::RwLock::new(RollingWindows::default()));
                    ...
                    for (pending, _) in batched {
                        ...
                        *aggregate_buffer.entry(key).or_insert(0) += 1;
                        ...
                        if matches!(pending.classification_type, ClassificationType::RouteLeak | ClassificationType::MinorRouteLeak | ClassificationType::Hijack | ClassificationType::Outage) {
                            rw_ingest.write().add_event(pending.lat, pending.lon, pending.asn, pending.as_name.clone(), c_ingest.get_as_org(pending.asn), pending.country.clone(), pending.city.clone(), pending.classification_type, now, pending.prefix.clone());
                        }
